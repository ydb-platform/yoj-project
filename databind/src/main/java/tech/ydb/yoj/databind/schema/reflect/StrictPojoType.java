...
/**
 * POJO with an all-args constructor: either explicitly annotated with {@link ConstructorProperties @ConstructorProperties}, or nullary,
 * or containing parameter names (compiled with {@code -parameter} argument of {@code javac}).
 */
/*package*/ final class StrictPojoType<T> implements ReflectType<T> {
    public static final StdReflector.TypeFactory FACTORY = new StdReflector.TypeFactory() {
        ...
        public <R> ReflectType<R> create(Reflector reflector, Class<R> rawType, FieldValueType fvt) {
            return new StrictPojoType<>(reflector, rawType);
        }
    };
    ...
    public StrictPojoType(@NonNull Reflector reflector, @NonNull Class<T> type) {
        ...
        if (propNamesAnnotation != null) {
            ...
            this.fields = Stream.of(type.getDeclaredFields())
                    .filter(StrictPojoType::isEntityField)
