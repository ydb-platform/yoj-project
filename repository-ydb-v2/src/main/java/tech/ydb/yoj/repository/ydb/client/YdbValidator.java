...
import tech.ydb.yoj.repository.ydb.exception.YdbSessionBusyException;
...
import static tech.ydb.yoj.repository.ydb.client.YdbSessionManager.REQUEST_GET_SESSION;
...
public final class YdbValidator {
    ...
    public static void validate(String request, Status status, String response) {
        ...
        switch (statusCode) {
            ...
            // Another query is being executed in this session. Create a new session with fast exponential backoff
            case SESSION_BUSY -> throw new YdbSessionBusyException(statusCode, request, response);
            ...
            case PRECONDITION_FAILED -> {
                if (is(issues, IssueCode.CONSTRAINT_VIOLATION::matches)) {
                    ...
                    throw new YdbResultSetTooBigException(statusCode, request, response);
                } else {
                    throw new YdbPreconditionFailedException(statusCode, request, response);
                }
            }
            ...
            case CLIENT_DEADLINE_EXPIRED -> {
                ...
                if (REQUEST_GET_SESSION.equals(request)) {
                    log.warn("""
