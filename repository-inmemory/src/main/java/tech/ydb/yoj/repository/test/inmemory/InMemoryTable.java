...
public class InMemoryTable<T extends Entity<T>> implements Table<T> {
    ...
    public T find(Entity.Id<T> id) {
        ...
        return transaction.getTransactionLocal().firstLevelCache().get(tableDescriptor, id, __ -> {
            markKeyRead(id);
    ...
    public <V extends View> V find(Class<V> viewType, Entity.Id<T> id) {
        ...
        if (cache.containsKey(tableDescriptor, id)) {
            return cache.peek(tableDescriptor, id)
                    .map(entity -> toView(viewType, schema, entity))
                    .orElse(null);
        }
    ...
    public T insert(T tt) {
        ...
        transaction.getTransactionLocal().firstLevelCache().put(tableDescriptor, t);
    ...
    public void delete(Entity.Id<T> id) {
        ...
        transaction.getTransactionLocal().firstLevelCache().putEmpty(tableDescriptor, id);
